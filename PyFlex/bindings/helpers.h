...
// need to be changed into class and Vec3
float get_distance(float start_x, float start_y, float start_z, float end_x, float end_y, float end_z)
{
	float dis = sqrt(pow(end_x-start_x, 2)+pow(end_y-start_y, 2)+pow(end_z-start_z, 2));
	return dis;
}

Vec3 get_direction(float start_x, float start_y, float start_z, float end_x, float end_y, float end_z, float dis)
{
	Vec3 dir = Vec3((end_x-start_x)/ dis, (end_y-start_y)/ dis, (end_z-start_z)/ dis);
	return dir;
}


float get_inv_mass(int num_node, float * node_ptr, int num_edge, float * edge_ptr, float rest_length, float mass)
{
	float length = 0;
	for(int i=0;i<num_edge;++i)
	{
		int start_node = edge_ptr[i*2];
		int end_node = edge_ptr[i*2+1];

		float start_x = node_ptr[start_node*3];
		float start_y = node_ptr[start_node*3+1];
		float start_z = node_ptr[start_node*3+2];

		float end_x = node_ptr[end_node*3];
		float end_y = node_ptr[end_node*3+1];
		float end_z = node_ptr[end_node*3+2];

		float dis = get_distance(start_x, start_y, start_z, end_x, end_y, end_z);
		length += dis;
	}
	// printf('num node is %f', length/rest_length);
	float inv_mass = 1/(mass/(length/rest_length));
	return inv_mass;
}
	...
	float rest_length, int phase, float mass=1.0f)
{
	...
	for(int i=0; i<num_edge; ++i)
	{
		...
		if (visited)
		{
			...
			if (ini_dis > 1.5*rest_length)
			{
				current_dis = get_distance(p.x, p.y, p.z, end_x, end_y, end_z);
				num_from_start = 1;
				while (current_dis > 1.5*rest_length)
				{				
					...
					if (num_from_start != 1)
					{
						if (num_from_start==2)
						{
							CreateSpring(node_idx[start_node], prev+1, bendStiffness*0.5f, give);
						}
						else
						{
				...
				// determine if the end node is visited
				visited_node.push_back(end_node);
				end_visited = set<int>(visited_node.begin(), visited_node.end()).size()!=visited_node.size();
				if (end_visited)
				{
					visited_node.pop_back();
					// stretch
					CreateSpring(prev, node_idx[end_node], stretchStiffness, give);
					// bend
					CreateSpring(prev-1, node_idx[end_node], bendStiffness*0.5f, give);
				}
				else
				{
					num_particles = g_buffers->positions.size();
					node_idx[end_node] = num_particles;
		...
		else // the start node is not visited
		{
			num_particles = g_buffers->positions.size();
			node_idx[start_node] = num_particles;
